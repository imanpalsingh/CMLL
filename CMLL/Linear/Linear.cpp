...
int LogisticRegression::model(const data::STORAGE &X, const data::STORAGE &y)
{
  ...
  /*  Calculating z */
   ...
   // W inverse
   auto W_inverse = matrix::inverse_diagonal(W);

   // y - p 
   auto y_minus_p = matrix::subtract(y,p);

   // W^{-1} * (y-p)
   auto w_inverse_mul_y_minus_p = matrix::multiply(W_inverse,y_minus_p);

   // z = X*B + W^{-1} * (y-p) 
   auto z = matrix::sum(B_hat,w_inverse_mul_y_minus_p);
  
  /*  Calculating B^{new} */

  //X transpose
  auto X_T = matrix::transpose(X);
  
  // X.T*W
  auto X_T_mul_W = matrix::multiply(X_T,W);

  // X.T*W*X
  auto X_T_mul_W_mul_X = matrix::multiply(X_T_mul_W,X);

  // Inverse of  X.T*W*X
  auto X_T_muk_W_mul_X_inverse = matrix::inverse(matrix::LU(X_T_mul_W_mul_X));

  // Inverse of X.T*W*X * X.T
  auto X_T_muk_W_mul_X_inverse_mul_X_T = matrix::multiply(X_T_muk_W_mul_X_inverse,X_T);

  // Inverse of X.T*W*X * X.T * W
  auto X_T_muk_W_mul_X_inverse_mul_X_T_mul_W = matrix::multiply(X_T_muk_W_mul_X_inverse_mul_X_T,W);

  // Inverse of X.T*W*X * X.T * W * z
  B  = matrix::multiply(X_T_muk_W_mul_X_inverse_mul_X_T_mul_W,z);
...
* Function Name : _multiclass_model

* Description : Function to build a logistic regression model for multiclass classification
...
* Note : This function is not to be called externally . Set the multiclass attribute to multiclass;

*
*/

int LogisticRegression::_multiclass_model(data::STORAGE X,data::STORAGE y)
{
    
    /*
    * Read function level header comments of model() for process information
    * This function will create multiple logistic models for each class
    */
