...
namespace csv{
    Handler read(std::string filename,const char seperator=',');
...
class Handler
{
    ...
    File with the filename specified is opened for reading. Each row is parsed with the seperators defined.
    Binary files are not supported since they require different mechanism 

    Paramteres 
    filename - >  A valid filepath for the file to be read
    seperator -> The value seperator 

    For example in a csv file  [   1,2,3,4 ] = ',' is the seperator
    return type : Handler
    */
    friend Handler csv::read(std::string filename,const char seperator);
