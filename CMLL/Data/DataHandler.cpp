...
* Parameters : filename -> The filename of the file to read.
             : seperator -> The delimeter that separates different observations in thefile (default = ',') 
...
Handler csv::read(std::string filename,const char seperator)
{
    ...
    if(!file_read)
    {
        std::cout<<"<In function csv::read> Error : Cannot open file sepcified.\n";
        file_read.close();
        return object;
    }
    ...
    while(file_read>>data)
    {
        ...
        // If the read character is the seperator of column or of line
        if((character == seperator) || (character == '\n'))
        {
        ...
        // If the character received is unkown 
            ...
            std::cout<<"<In function csv::read> Error : An unspecified character '"<<character<<"' was received at row 1 column "<<columns_num<<" which was expected to be the delimeter '"<<seperator<<"'\n";
            file_read.close();
            return object;
        }
    ...
    while(file_read>>data)
    {
        ...
        // If the character received is unkown 
            ...
            std::cout<<"<In function file_read> Error : An unspecified character '"<<character<<"' was received at row 1 column "<<columns_num<<" which was expected to be the delimeter '"<<seperator<<"'\n";
            file_read.close();
            return object;
        }
